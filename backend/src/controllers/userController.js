    ...
    try {
        ...
        const recommendedUsers=await User.find({
            $and:[
                ...
                {_id:{$nin:currentUser.friends}},//exculde current user's friend
                {isOnboarded:true}
            ]
...
export async function getMyFriends(req,res){
    try {
        ...
        .populate("friends","fullName bio profilePic nativeLanguage learningLanguage");
        res.status(200).json(user.friends);
    } catch (err) {
        console.log("error in friends controller",err.message);
        res.status(500).json({message:"Internal Server Error"});
    }
...
export async function acceptFriendRequest(req,res){
    try {
        ...
        //$addtoset adds elements to an array only if they dont already exists.
...
export async function getOutgoingFriendReqs(req,res){
    try {
        ...
        console.log("Error in getOutgoingFriendRequest controller", err.message);
        res.status(500).json({ message: "Internal Server Error" });
